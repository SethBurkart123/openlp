...
class MCPWorker(QtCore.QObject):
    ...
    create_service_requested = QtCore.pyqtSignal()
    load_service_requested = QtCore.pyqtSignal(str)  # file_path
    save_service_requested = QtCore.pyqtSignal(str)  # file_path (optional)
    get_service_items_requested = QtCore.pyqtSignal()
    add_custom_slide_requested = QtCore.pyqtSignal(str, str)  # title, content
    add_custom_slides_requested = QtCore.pyqtSignal(str, list, str, int)  # title, slides, credits, position
    add_media_requested = QtCore.pyqtSignal(str, str, int)  # file_path, title, position
    go_live_requested = QtCore.pyqtSignal(int)  # item_index
    next_slide_requested = QtCore.pyqtSignal()
    previous_slide_requested = QtCore.pyqtSignal()
    list_themes_requested = QtCore.pyqtSignal()
    set_theme_requested = QtCore.pyqtSignal(str)  # theme_name
    create_from_structure_requested = QtCore.pyqtSignal(object)  # service_structure
    
    # Plugin search and add signals
    search_songs_requested = QtCore.pyqtSignal(str)  # text
    create_song_requested = QtCore.pyqtSignal(str, str, str)  # title, lyrics, author
    add_song_by_id_requested = QtCore.pyqtSignal(int, int)  # song_id, position
    
    # Theme management signals
    create_theme_requested = QtCore.pyqtSignal(object)  # theme_data
    get_theme_details_requested = QtCore.pyqtSignal(str)  # theme_name
    update_theme_requested = QtCore.pyqtSignal(object)  # update_data
    delete_theme_requested = QtCore.pyqtSignal(str)  # theme_name
    duplicate_theme_requested = QtCore.pyqtSignal(str, str)  # existing_theme_name, new_theme_name
    
    # Service item positioning and slide management signals
    move_service_item_requested = QtCore.pyqtSignal(int, int)  # from_index, to_index
    remove_service_item_requested = QtCore.pyqtSignal(int)  # index
    get_service_item_slides_requested = QtCore.pyqtSignal(int)  # item_index
    
    # Per-item theme management signals
    set_item_theme_requested = QtCore.pyqtSignal(int, str)  # item_index, theme_name
    get_item_theme_requested = QtCore.pyqtSignal(int)  # item_index
    
    # Result signals
    operation_completed = QtCore.pyqtSignal(object)  # result
    ...
    @QtCore.pyqtSlot(str, list, str, int)
    ...
    def add_song_by_id(self, song_id, position):
        ...
        try:
            ...
            mock_item.setData(Qt.UserRole, song_id)
